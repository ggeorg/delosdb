 ...
 * production module points directly at {@code AsmJava}, and generated bytecode
 * is emitted as Java 21 classfiles.</p>
 */
public final class GeneratedBytecodeAsmJvm21Proof {
    ...
    public static void main(String[] args) throws Exception {
        assertProductionModuleUsesAsmJava();
        byte[] classBytes = generateThroughDefaultCompiler();
        assertJava21Classfile(classBytes);
        assertClassReaderSeesGeneratedClass(classBytes);
        assertGeneratedClassRuns(classBytes);

        System.out.println("Generated bytecode ASM JVM 21 proof passed: moduleCompiler=" + ASM_CLASS
                + " classfileMajor=" + Opcodes.V21);
    }

    private static void assertProductionModuleUsesAsmJava() throws Exception {
        Properties properties = new Properties();
        try (InputStream stream = openModulesProperties()) {
            properties.load(stream);
        }
        String compiler = properties.getProperty(MODULE_PROPERTY);
        if (!ASM_CLASS.equals(compiler)) {
            throw new AssertionError("Expected production " + MODULE_PROPERTY + " to use " + ASM_CLASS
                    + " but was " + compiler);
        }
    }
    ...
    private static byte[] generateThroughDefaultCompiler() throws Exception {
