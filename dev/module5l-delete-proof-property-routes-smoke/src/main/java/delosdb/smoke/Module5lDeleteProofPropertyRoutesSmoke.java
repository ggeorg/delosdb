package delosdb.smoke;

import io.github.ggeorg.delosdb.engine.extension.storage.versioned.sql.DelosNativeTableRegistry;
import org.apache.derby.impl.sql.execute.DelosTableScanProviderLookup;

import java.nio.file.Path;
import java.sql.Connection;
import java.sql.ResultSet;
import java.sql.Statement;
import java.util.ArrayList;
import java.util.List;

/**
 * MODULE5L smoke: old native MVCC CRUD proof-property routes are disabled.
 *
 * <p>The properties are intentionally set to {@code true} before running CRUD.
 * They remain as compatibility names only; provider identity must be the route
 * selector for delos_mvcc SELECT full scan, INSERT, UPDATE, and DELETE. Heap
 * tables must continue to use Derby's inherited heap path.</p>
 *
 * <p>This does not remove the transitional Delos ResultSet classes yet. It only
 * prevents the old Phase F/G property switches from being a second routing
 * truth.</p>
 */
public final class Module5lDeleteProofPropertyRoutesSmoke {
    private static final String DATABASE_PATH = "build/module5l-delete-proof-property-routes-db";
    private static final String MVCC_TABLE = "MODULE5L_MVCC";
    private static final String HEAP_TABLE = "MODULE5L_HEAP";

    private Module5lDeleteProofPropertyRoutesSmoke() {
    }

    public static void main(String[] args) throws Exception {
        SmokeUtils.shutdownQuietly(DATABASE_PATH);
        SmokeUtils.deleteRecursively(Path.of(DATABASE_PATH));
        DelosNativeTableRegistry.clearRegisteredTablesForTesting();
        clearLegacyProperties();
        poisonLegacyCrudProperties();
        assertLegacyPropertiesAreSet();
        assertLegacyCrudRoutesDisabled();

        SmokeUtils.loadEmbeddedDriver();
        try (Connection connection = SmokeUtils.connect(DATABASE_PATH, true);
             Statement statement = connection.createStatement()) {
            statement.executeUpdate("CREATE TABLE APP." + MVCC_TABLE
                    + " (id INT, name VARCHAR(32)) USING delos_mvcc");
            statement.executeUpdate("CREATE TABLE APP." + HEAP_TABLE
                    + " (id INT, name VARCHAR(32))");

            statement.executeUpdate("INSERT INTO APP." + MVCC_TABLE + " VALUES (1, 'one')");
            statement.executeUpdate("INSERT INTO APP." + MVCC_TABLE + " VALUES (2, 'two')");
            SmokeUtils.assertEquals(List.of(1, 2), ids(statement, MVCC_TABLE),
                    "delos_mvcc INSERT/SELECT must route by provider identity even when old proof properties are set");

            statement.executeUpdate("UPDATE APP." + MVCC_TABLE + " SET name = 'updated'");
            SmokeUtils.assertEquals(List.of("updated", "updated"), names(statement, MVCC_TABLE),
                    "delos_mvcc UPDATE must route by provider identity, not the old update property");

            SmokeUtils.assertEquals(2,
                    statement.executeUpdate("DELETE FROM APP." + MVCC_TABLE),
                    "delos_mvcc DELETE must route by provider identity, not the old delete property");
            SmokeUtils.assertEquals(List.of(), ids(statement, MVCC_TABLE),
                    "delos_mvcc DELETE result must stay visible through provider identity routing");

            statement.executeUpdate("INSERT INTO APP." + HEAP_TABLE + " VALUES (10, 'heap')");
            SmokeUtils.assertEquals(List.of(10), ids(statement, HEAP_TABLE),
                    "heap table must still work while old delos_mvcc proof properties are inert");
            SmokeUtils.assertEquals(1,
                    statement.executeUpdate("UPDATE APP." + HEAP_TABLE + " SET name = 'heap-updated'"),
                    "heap UPDATE must keep using Derby heap path");
            SmokeUtils.assertEquals(List.of("heap-updated"), names(statement, HEAP_TABLE),
                    "heap SELECT must keep using Derby heap path");
            SmokeUtils.assertEquals(1,
                    statement.executeUpdate("DELETE FROM APP." + HEAP_TABLE),
                    "heap DELETE must keep using Derby heap path");
            SmokeUtils.assertEquals(List.of(), ids(statement, HEAP_TABLE),
                    "heap DELETE must remove the heap row normally");
        } finally {
            clearLegacyProperties();
            DelosNativeTableRegistry.clearRegisteredTablesForTesting();
            SmokeUtils.shutdownQuietly(DATABASE_PATH);
        }
    }

    private static List<Integer> ids(Statement statement, String tableName) throws Exception {
        List<Integer> ids = new ArrayList<>();
        try (ResultSet rows = statement.executeQuery("SELECT id, name FROM APP." + tableName)) {
            while (rows.next()) {
                ids.add(rows.getInt(1));
            }
        }
        ids.sort(Integer::compareTo);
        return List.copyOf(ids);
    }

    private static List<String> names(Statement statement, String tableName) throws Exception {
        List<String> names = new ArrayList<>();
        try (ResultSet rows = statement.executeQuery("SELECT id, name FROM APP." + tableName)) {
            while (rows.next()) {
                names.add(rows.getString(2));
            }
        }
        names.sort(String::compareTo);
        return List.copyOf(names);
    }

    private static void poisonLegacyCrudProperties() {
        System.setProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_INSERT_PROPERTY, "true");
        System.setProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_SELECT_ALL_PROPERTY, "true");
        System.setProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_DELETE_EQUALITY_PROPERTY, "true");
        System.setProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_UPDATE_EQUALITY_PROPERTY, "true");
    }

    private static void assertLegacyPropertiesAreSet() {
        assertPropertySet(DelosTableScanProviderLookup.FACTORY_NATIVE_INSERT_PROPERTY);
        assertPropertySet(DelosTableScanProviderLookup.FACTORY_NATIVE_SELECT_ALL_PROPERTY);
        assertPropertySet(DelosTableScanProviderLookup.FACTORY_NATIVE_DELETE_EQUALITY_PROPERTY);
        assertPropertySet(DelosTableScanProviderLookup.FACTORY_NATIVE_UPDATE_EQUALITY_PROPERTY);
    }

    private static void assertPropertySet(String propertyName) {
        if (!Boolean.getBoolean(propertyName)) {
            throw new AssertionError("MODULE5L expected legacy proof property to be set as poison input: "
                    + propertyName);
        }
    }

    private static void assertLegacyCrudRoutesDisabled() {
        if (DelosTableScanProviderLookup.legacyNativeMvccCrudProofRoutesEnabledForTesting()) {
            throw new AssertionError("MODULE5L must keep old native MVCC CRUD proof properties as inert names only");
        }
    }

    private static void clearLegacyProperties() {
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_PROBE_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_INSERT_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_SELECT_ALL_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_SELECT_EQUALITY_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_RANGE_PREDICATES_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_BETWEEN_PREDICATES_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_NULL_PREDICATES_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_OR_PREDICATES_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_PROJECTION_VARIANTS_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_ORDER_BY_RESIDUAL_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_COUNT_AGGREGATE_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_DELETE_EQUALITY_PROPERTY);
        System.clearProperty(DelosTableScanProviderLookup.FACTORY_NATIVE_UPDATE_EQUALITY_PROPERTY);
    }
}
