...
/**
 * MODULE5F proof smoke: Derby DELETE and UPDATE route to delos_mvcc by provider identity.
 *
 * <p>The historical F6/F7 native DELETE/UPDATE proof properties are deliberately
 * left unset. Normal Derby DELETE and UPDATE statements over a {@code USING
 * delos_mvcc} table must reach the native MVCC mutation path because the catalog
 * table identity says the table is provider-backed. A default heap table in the
 * same database must still use the normal Derby heap DELETE/UPDATE path.</p>
 */
public final class Module5fDerbyDeleteUpdateProviderRouteSmoke {
    private static final String DATABASE_PATH = "build/module5f-derby-delete-update-provider-route-db";
    private static final String MVCC_TABLE = "MODULE5F_DML_MVCC";
    private static final String HEAP_TABLE = "MODULE5F_DML_HEAP";

    private Module5fDerbyDeleteUpdateProviderRouteSmoke() {
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (Connection connection = SmokeUtils.connect(DATABASE_PATH, true);
            ...
            assertNativeDeleteUpdatePropertiesAreNotSet();
            SmokeUtils.assertEquals(2,
                    statement.executeUpdate("UPDATE APP." + MVCC_TABLE + " SET name = 'mvcc-updated'"),
                    "Derby UPDATE should modify delos_mvcc rows without the old proof property");
            SmokeUtils.assertEquals(List.of("mvcc-updated", "mvcc-updated"), names(statement, MVCC_TABLE),
                    "Derby UPDATE should preserve updated delos_mvcc row values");

            SmokeUtils.assertEquals(2,
                    statement.executeUpdate("DELETE FROM APP." + MVCC_TABLE),
                    "Derby DELETE should delete delos_mvcc rows without the old proof property");
            SmokeUtils.assertEquals(List.of(), ids(statement, MVCC_TABLE),
                    "Derby DELETE should make deleted delos_mvcc rows invisible");
            ...
            SmokeUtils.assertEquals(List.of("heap-updated"), names(statement, HEAP_TABLE),
                    "default heap UPDATE should preserve heap row values");
            SmokeUtils.assertEquals(1,
    ...
    private static void assertNativeDeleteUpdatePropertiesAreNotSet() {
        if (Boolean.getBoolean(DelosTableScanProviderLookup.FACTORY_NATIVE_DELETE_EQUALITY_PROPERTY)) {
            throw new AssertionError("MODULE5F must not rely on the old native DELETE proof property");
        }
        if (Boolean.getBoolean(DelosTableScanProviderLookup.FACTORY_NATIVE_UPDATE_EQUALITY_PROPERTY)) {
            throw new AssertionError("MODULE5F must not rely on the old native UPDATE proof property");
        }
    }
