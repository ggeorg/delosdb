...
prepared.  `DelosMutationPreparation` does not expose lock state; Option A is a
non-locking contract by structure, not by a runtime guard around a false field.
...
preparation exposes no lock-acquisition state
...
the native Derby mutation boundary.  The 40001 SQLState is used as Derby's
transaction/serialization conflict surface for this write/write conflict; it is
not a true deadlock-detector result.
