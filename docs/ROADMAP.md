...
## Completed lane: F-I native `delos_mvcc` execution closeout

The F-I native execution lane is closed.  The retired SQL bridge is gone from
the active source surface, and supported `delos_mvcc` SQL now flows through
Derby parser/binder/optimizer/generated execution, `ResultSetFactory`, native
Delos result sets, and `DelosNativeTableRegistry`.

Closed sequence:

```text
Phase F — native Derby execution path: provider syntax, catalog metadata,
          ResultSetFactory branch, native SELECT/INSERT/DELETE/UPDATE
Phase G — native predicate/index coverage: AND predicates, range, BETWEEN,
          SELECT *, COUNT(*), CREATE INDEX, bridge fallback retirement
Post-G  — native table registry extraction and restart/reopen proof
Phase H — cost observation, heap proof-cost mapping, session cost tuning
Phase I — Option A optimistic mutation preparation and 40001 conflict mapping
```

Active verification is intentionally compact:

```text
storagePhaseC7StabilizationSmoke
verifyStoragePhaseGConsolidation
verifyStoragePhaseH4SessionCostConstant
verifyStoragePhaseI2MutationConflict
verifyStorageNativeExecutionCloseout
verifyDelosDbPermanentStorageGuards
```

Historical per-step F/G/H/I smoke ladders and phase Gradle files are retired.
The remaining `dev/` storage smokes are closeout witnesses, not a phase-by-phase
...
Closed C/F text-token report guards and historical F/G/H/I phase-step smoke
wiring are retired from active build wiring. Permanent storage boundary guards,
SPI truth-map checks, closed C7 regression smoke, and the compact native
execution closeout smokes remain active.
