...
The current build has extracted compile and runtime jar ownership from the repository root into Gradle subprojects. `delosdb-osgi-stub`, `delosdb-commons`, `delosdb-engine`, `delosdb-client`, `delosdb-tools`, `delosdb-runner`, `delosdb-optionaltools`, and `delosdb-server` now own their compile lifecycles and runtime jars. The root build coordinates shared resource processing, message splitting, build-tool generation, and product-level verification. The table below is the authoritative inventory for the next extraction steps.
...
| `osgi-framework-stub.jar` | `:delosdb-osgi-stub` |
