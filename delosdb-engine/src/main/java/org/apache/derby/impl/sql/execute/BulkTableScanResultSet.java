...
outer:		for (;;)
	...
	*/
    ...
    boolean supportsIndexToBaseBatch() {
        return !fetchRowLocations && rowsPerRead > 1;
    }

    int indexToBaseBatchCapacity() {
        return Math.min(rowsPerRead, 256);
    }

    int indexToBaseBatchWidth() throws StandardException {
        if (currentRow == null) {
            currentRow = getCompactRow(candidate, accessedCols, isKeyed);
        }
        return currentRow.nColumns();
    }

    /**
     * Fill one bounded consumer batch with shallow references from the current
     * store group. The method never reloads a store group after accepting a
     * qualifying row, so every returned value remains stable until the caller
     * asks for the next batch.
     */
    int fetchIndexToBaseBatch(DataValueDescriptor[][] destination)
            throws StandardException {
        if (isXplainOnlyMode()) {
            return 0;
        }
        if (SanityManager.DEBUG) {
            SanityManager.ASSERT(supportsIndexToBaseBatch(),
                    "index-to-base batch requested from unsupported bulk scan");
        }
        ...
        if (isOpen && scanControllerOpened) {
            ...
            while (count == 0) {
                ...
                while (++curRowPosition < numRowsInArray
                    ...
                    DataValueDescriptor[] sourceValues = currentRow.getRowArray();
                    if (SanityManager.DEBUG) {
                        SanityManager.ASSERT(
                                destination[count].length == sourceValues.length,
                                "index-to-base batch width mismatch");
                    }
                    System.arraycopy(
                            sourceValues, 0, destination[count], 0, sourceValues.length);
                    count++;
                }

                // Once a row from a store group is exposed, return before any
                // reload can recycle that group's value holders.
                if (count > 0) {
                    break;
                }
            }
        ...
        setCurrentRow(count == 0 ? null : currentRow);
        nextTime += getElapsedMillis(beginTime);
        return count;
    }
