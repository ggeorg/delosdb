...
final class MvccRawStoreOrderedIndex {
    ...
    static Optional<CandidateCursor> openCandidateCursorForAt(
            TransactionManager transactionManager,
            MvccRawStoreTable.Descriptor table,
            ContainerKey directoryKey,
            Qualifier[][] qualifiers,
        ...
        return Optional.of(openCandidateCursor(
                transactionManager,
                table,
                btree,
                predicate,
    ...
    private static CandidateCursor openCandidateCursor(
            TransactionManager transactionManager,
            MvccRawStoreTable.Descriptor table,
        ...
        return new CandidateCursor(scan, row, predicate.columnId());
    }

    static final class CandidateCursor implements AutoCloseable {
        private final ScanController scan;
        private final StoreDataValue[] row;
        private final int columnId;
        private final java.util.HashSet<Long> seenRowIds = new java.util.HashSet<>();
        ...
        List<Candidate> nextBatch(int maxCandidates) throws StandardException {
            if (maxCandidates <= 0) {
                throw new IllegalArgumentException(
                        "RawStore MVCC candidate batch size must be positive");
            }
            if (closed) {
                return List.of();
            }
            java.util.ArrayList<Candidate> batch = new java.util.ArrayList<>(maxCandidates);
            while (batch.size() < maxCandidates && scan.fetchNext(row)) {
                ...
                MvccRowLocation rowLocation = MvccRowLocation.from(row[ROW_LOCATION_FIELD]);
        ...
        public void close() {
            if (!closed) {
                ...
                try {
                    ...
                    throw new IllegalStateException(
                            "RawStore MVCC ordered-index candidate scan close failed",
                            failure);
                }
            }
        }
